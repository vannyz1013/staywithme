...
// worse -- the durable parts are in memory and traits (see app/src/memory
// and character evolution), and this window is only the thread of the
// current exchange.
...
export function windowFor(messages: StoredMessage[]): WireMessage[] {
  return messages.slice(-WINDOW_SIZE).map((message) => ({
    role: message.role,
    text: message.text,
